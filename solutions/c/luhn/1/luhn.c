    ...
    for (i = (int)strlen(num) - 1; i >= 0; i--) {
        ...
        if (j%2==0) { // Checks if it's a number which needs to be doubled
            if ((num[i]-48)*2 > 9) { // Checks if said number goes into two digits
                arr[j-1] = (num[i]-48)*2 - 9; // Removes 9, which equates to the process of removing 10 in order to get the                                                          last digit, and then adding 1 to account for the tens place.
            } else {
                arr[j-1] = (num[i]-48)*2; // Just sets the double of the number
            }
        } else {
            arr[j-1] = (num[i]-48); // Just sets the number
        }
    }
