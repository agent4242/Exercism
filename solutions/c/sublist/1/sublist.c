...
comparison_result_t check_lists(int *list_to_compare, int *base_list,
    ...
    size_t common_index;
    COMPARISON:
    for (; i < bigcount; i++) {    // Searches for the first element of the smaller set in the bigger one
        if (smallerlist[0] == biggerlist[i]) {
            if ((bigcount - i) < smallcount) return UNEQUAL;    // If the remaining elements in the bigger set are less than
            unequal = false;                                    // the total count of the smaller set, immiedetely return
            i++;                                                // unequal.
            common_index = i;
            break;                                              
        }
